...
RUN apt-get -y install openjdk-11-jdk wget unzip
...
RUN cd /tmp && tar xvfz tomcat.tar.gz && unzip main.zip
RUN cp -Rv /tmp/apache-tomcat-10.0.4/** /usr/local/tomcat/
RUN cp -rf /tmp/tomcattest-main/index.jsp /usr/local/tomcat/webapps/ROOT/
